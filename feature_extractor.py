...
def domain_adaptation(unet, config, fine_tune):
    ...
    if fine_tune:      
        ...
        if config.model.DDADS==True:
            torch.save(frozen_feature_extractor.state_dict(), os.path.join(config.data.data_dir,"train", config.data.category,str(config.model.w),str(config.model.epochs),"smallUnet",f'feat0'))
        else:
            torch.save(frozen_feature_extractor.state_dict(), os.path.join(config.data.data_dir,"train", config.data.category,str(config.model.w),str(config.model.epochs),"bigUnet",f'feat0'))
        ...
        for epoch in range(config.model.DA_epochs):
            loss = torch.tensor(0.0, device=config.model.device)  # 初始化 loss
            for step, batch in enumerate(trainloader):
            ...
            if (epoch+1) % 4 == 0:
                if config.model.DDADS==True:
                    torch.save(feature_extractor.state_dict(), os.path.join(os.path.join(config.data.data_dir),"train", config.data.category,str(config.model.w),str(config.model.DLlambda),"smallUnet",f'feat{epoch+1}'))
                else:
                    torch.save(feature_extractor.state_dict(), os.path.join(os.path.join(config.data.data_dir),"train", config.data.category,str(config.model.w),str(config.model.DLlambda),"bigUnet",f'feat{epoch+1}'))
        # torch.save(feature_extractor.state_dict(), os.path.join(os.path.join(config.data.data_dir),"train", config.data.category,f'feat{epoch+1}'))
            # else:
                ...
                # checkpoint = torch.load(os.path.join(config.data.data_dir,"train", config.data.category,str(config.model.w),str(config.model.DLlambda),"",f'feat{config.model.DA_chp}'))#{config.model.DA_chp}
    else:
        if config.model.DDADS==True:
            checkpoint =torch.load(os.path.join(os.path.join(config.data.data_dir),"train", config.data.category,str(config.model.w),str(config.model.DLlambda),"smallUnet",f'feat{config.model.DA_chp}'))
        else:
            checkpoint =torch.load(os.path.join(os.path.join(config.data.data_dir),"train", config.data.category,str(config.model.w),str(config.model.DLlambda),"bigUnet",f'feat{config.model.DA_chp}'))  
