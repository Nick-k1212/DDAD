...
def heat_map(output, target, FE, config,v_value):
    '''
    ...
    s_d=SSIM_distance(output, target,config)

    # anomaly_map += f_d + config.model.v * (torch.max(f_d)/ torch.max(i_d)) * i_d
    anomaly_map += f_d + v_value * (torch.max(f_d)/ torch.max(i_d)) * i_d
    # anomaly_map+=s_d
...
def SSIM_distance(output, target, config):
    '''
    計算 SSIM 異常分數並產生 anomaly map
    '''
    output_np = output.detach().cpu().numpy().astype(np.float32)  # 轉換為 NumPy
    target_np = target.detach().cpu().numpy().astype(np.float32)  # 轉換為 NumPy

    print("output_np shape:", output_np.shape)
    print("target_np shape:", target_np.shape)

    batch_size, channels, height, width = output_np.shape  # 取得 batch 大小
    anomaly_map = torch.zeros((batch_size, 1, height, width)).to(config.model.device)  # 建立 anomaly_map

    for i in range(batch_size):
        ssim_value = ssim(
            output_np[i],  # 取出 batch 中的第 i 張影像
            target_np[i],  # 取出 batch 中的對應 target 影像
            channel_axis=0,  # 確保 RGB 通道正確處理
            data_range=1.0
        )
        
        ssim_value = torch.tensor(ssim_value, dtype=torch.float32, device=config.model.device)  # 轉為 Tensor
        ssim_value = ssim_value.view(1, 1, 1, 1).expand(1, 1, height, width)  # 轉換形狀，擴展成影像大小
        
        anomaly_map[i] = 1 - ssim_value  # 讓異常區域數值越大
